...
class XaiMusic:
    ...
    def __check_config_validity(self):
        ...
        """
        required_keys = [
            "augmented_audio_save_path",
            "mir_dataset_path",
            "tempo_factor",
            "keys",
            "model_tag_result_save_path",
        ]
    ...
    def __time_stretch(self, signal: ndarray, time_stretch_rate: float) -> ndarray:
        """
        Time Stretch
    ...
    def __pitch_scale(
        self, signal: ndarray, sr: float, num_semitones: float
    ) -> ndarray:
        """Pitch Shift
    ...
    def synthesis_audiobank(self, n_sample_genre: int = None) -> None:
        """
        ...
        """
        audio_file_paths = self.__get_audio_file_paths(self.config["mir_dataset_path"])
        ...
        if n_sample_genre is None:
            audio_lst = [
                os.path.join(self.config["mir_dataset_path"], row.type, row.audio)
            ...
            if n_sample_genre <= 0 or n_sample_genre > df_music_types["type"].nunique():
                ...
                for mt in music_types:
                    ...
                    audio_lst.extend(
                        [
                            os.path.join(
                                self.config["mir_dataset_path"], row.type, row.audio
                            )
        ...
        for i, wa in enumerate(tqdm(audio_lst, desc="Processing audio files")):
            ...
            for tmp_n, tmp_v in self.config["tempo_factor"].items():
                ...
                for key in self.config["keys"]:
                    augments_2_audio = self.__pitch_scale(augments_1_audio, sr, key)
                    key_note = "original" if key == 0 else f"{key}"
                    file_name = f"{m_type}-{base_file_name}|{tmp_n}|{key_note}{self.audio_format}"
                    sf.write(os.path.join(save_path, file_name), augments_2_audio, sr)

    def save_tag_result(self, pred_audio_path: str, pred_tags: list) -> None:
        """
        ...
        if not os.path.exists(pred_audio_path):
            raise FileNotFoundError(f"The audio file {pred_audio_path} does not exist.")
        ...
        audio_dict = {"tags": pred_tags, "music": music, "tempo": tempo, "key": key}
    ...
    def __plot_result(
        ...
        group_ohe_df = (
            group_ohe_df.apply(pd.to_numeric, errors="coerce", downcast="integer")
            .fillna(0)
            .astype(int)
        )
        ...
        ax.xaxis.set_major_locator(
            ticker.FixedLocator(np.arange(-0.5, len(x_labels_list), 1))
        )  # Adjusted for x-axis
        ax.yaxis.set_major_locator(
            ticker.FixedLocator(np.arange(-0.5, len(y_labels_list), 1))
        )  # Adjusted for y-axis
