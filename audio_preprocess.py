...
def downsample_mono(path, sr):
    ...
    try:
        ...
        raise exc
    # Corrected the resample call to only include the necessary arguments
...
def split_wavs(args):
    ...
    classes = [d for d in os.listdir(
        src_root) if os.path.isdir(os.path.join(src_root, d))]
    for _cls in classes:
        target_dir = os.path.join(dst_root, _cls)
        check_dir(target_dir)
        src_dir = os.path.join(src_root, _cls)
        for fn in tqdm([f for f in os.listdir(src_dir) if os.path.isfile(os.path.join(src_dir, f)) and f.endswith('.wav')]):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--fn', type=str, default='Albums-Cafe_Paradiso-06',
